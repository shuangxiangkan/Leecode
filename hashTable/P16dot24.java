	...
	public List<List<Integer>> pairSums(int[] nums, int target) {
		...
		for(int i:map.keySet()) {
			if(i==target/2&&target%i==0) {
				...
				int left=target-i;
				if(map.containsKey(left)&&labels.get(left)!=0) {
					int num=Math.min(map.get(i), map.get(left));
					for(int j=0;j<num;j++) {
						List<Integer> list=new ArrayList<>();
						list.add(i);
						list.add(left);
